...
public class EM_EventManager
{
	...
	public void onEntityJoinWorld(EntityJoinWorldEvent event)
	{
		...
		if(event.entity instanceof EntityLivingBase)
		{
			...
			boolean allowTracker = !(event.world.isRemote && EMINE.proxy.isClient() && Minecraft.getMinecraft().isIntegratedServerRunning());
			...
			if(EnviroDataTracker.isLegalType((EntityLivingBase)event.entity) && (event.entity instanceof EntityPlayer || EM_Settings.trackNonPlayer) && allowTracker)
			{
				...
				if(!hasOld)
				{
					...
					emTrack.loadNBTTags();
					if(!EMINE.proxy.isClient() || EMINE.proxy.isOpenToLAN())
					{
				...
				if (event.entity instanceof EntityPlayerMP && !event.world.isRemote) 
				{
					...
					EMINE.instance.network.sendTo(new PacketEnviroMine(pData), (EntityPlayerMP) event.entity);
...
//						event.useItem = Result.DENY;
		...
		} else if(event.getResult() != Result.DENY && event.action == Action.RIGHT_CLICK_AIR && item == null)
		{
			...
			EMINE.instance.network.sendToServer(new PacketEnviroMine(pData));
		}
	}
	...
	public void onLivingUpdate(LivingUpdateEvent event)
	{
		...
		if(event.entityLiving instanceof EntityPlayer)
		{
			...
			if(EM_Settings.enableAirQ && EM_Settings.enableBodyTemp && EM_Settings.enableHydrate && EM_Settings.enableSanity && EM_Settings.enablePhysics)
			{
				int seaLvl = 48;
		...
		if(tracker == null || tracker.isDisabled)
		{
			if((!EMINE.proxy.isClient() || EMINE.proxy.isOpenToLAN()) && (EM_Settings.enableAirQ || EM_Settings.enableBodyTemp || EM_Settings.enableHydrate || EM_Settings.enableSanity))
			{
				...
				{
					EMINE.logger.log(Level.WARN, "Server lost track of player! Attempting to re-sync...");
					EnviroDataTracker emTrack = new EnviroDataTracker((EntityLivingBase)event.entity);
					EM_StatusManager.addToManager(emTrack);
					emTrack.loadNBTTags();
					EM_StatusManager.syncMultiplayerTracker(emTrack);
		...
		if(event.entityLiving instanceof EntityPlayer)
		{
			...
			} else if(tracker != null && event.entityLiving.worldObj.isDaytime())
			{
				...
				if(tracker.sleepState.equals("Asleep") && tracker.lastSleepTime - relitiveTime > 100)
				{
					...
					{
						EMINE.logger.log(Level.ERROR, "Something went wrong while calculating sleep sanity gain! Result: " + timeSlept);
						tracker.sanity = 100;
