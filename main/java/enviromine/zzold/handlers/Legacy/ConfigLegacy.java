...
public class ConfigLegacy extends LegacyHandler
{
	...
	public boolean initCheck() 
	{
		...
		if(configFile.exists() && !configFile.isDirectory())
		{
			...
			{
				EMINE.logger.log(Level.WARN, "Failed to load Legacy configuration file!", e);
				return false;
			}
			
		    EMINE.logger.log(Level.INFO, "Legacy: Config File Loaded");
			return true;
		}
		else
		{
	...
	private static void MoveCustomProperties()
	{
		...
		if(customsDir.exists() && customsDir.listFiles().length > 0)
		{
			...
			{
				EMINE.logger.log(Level.ERROR, "Legacy Failed to Copy Custom Configs to New Dir! " + e);
			}
	...
	{	
		...
		try {
			...
			EMINE.logger.log(Level.ERROR, "Legacy Tried to Remove Enviromine.cfg... But failed! " +  e);
		}
