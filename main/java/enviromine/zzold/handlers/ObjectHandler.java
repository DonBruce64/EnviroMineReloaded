...
public class ObjectHandler
{
	...
	{
		badWaterBottle = new EnviroItemBadWaterBottle().setMaxStackSize(1).setUnlocalizedName("enviromine.badwater").setCreativeTab(EMINE.enviroTab);
		saltWaterBottle = new EnviroItemSaltWaterBottle().setMaxStackSize(1).setUnlocalizedName("enviromine.saltwater").setCreativeTab(EMINE.enviroTab);
		coldWaterBottle = new EnviroItemColdWaterBottle().setMaxStackSize(1).setUnlocalizedName("enviromine.coldwater").setCreativeTab(EMINE.enviroTab);
		airFilter = new Item().setMaxStackSize(16).setUnlocalizedName("enviromine.airfilter").setCreativeTab(EMINE.enviroTab).setTextureName("enviromine:air_filter");
		rottenFood = new RottenFood(1).setMaxStackSize(64).setUnlocalizedName("enviromine.rottenfood").setCreativeTab(EMINE.enviroTab).setTextureName("enviromine:rot");
		spoiledMilk = new ItemSpoiledMilk().setUnlocalizedName("enviromine.spoiledmilk").setCreativeTab(EMINE.enviroTab).setTextureName("bucket_milk");
		...
		hardHat = (ItemArmor)new EnviroArmor(camelPackMaterial, 4, 0).setTextureName("hard_hat").setUnlocalizedName("enviromine.hardhat").setCreativeTab(EMINE.enviroTab);
	}
	...
	public static void registerItems()
	{
		...
		EMINE.enviroTab.addRawStack(camelStack1);
		...
		EMINE.enviroTab.addRawStack(camelStack2);
		...
		EMINE.enviroTab.addRawStack(mask);
	...
	public static void initBlocks()
	{
		...
		gasBlock = new BlockGas(gasMat).setBlockName("enviromine.gas").setCreativeTab(EMINE.enviroTab).setBlockTextureName("enviromine:gas_block");
		fireGasBlock = new BlockGas(gasMat).setBlockName("enviromine.firegas").setCreativeTab(EMINE.enviroTab).setBlockTextureName("enviromine:gas_block").setLightLevel(1.0F);

		elevatorMat = new MaterialElevator(MapColor.ironColor);
		elevator = new BlockElevator(elevatorMat).setBlockName("enviromine.elevator").setCreativeTab(EMINE.enviroTab).setBlockTextureName("iron_block");
		
		davyLampBlock = new BlockDavyLamp(Material.redstoneLight).setLightLevel(1.0F).setBlockName("enviromine.davy_lamp").setCreativeTab(EMINE.enviroTab);
		davyLamp = new ItemDavyLamp(davyLampBlock).setUnlocalizedName("enviromine.davylamp").setCreativeTab(EMINE.enviroTab);
		
		flammableCoal = new BlockFlammableCoal();
		burningCoal = new BlockBurningCoal(Material.rock).setBlockName("enviromine.burningcoal").setCreativeTab(EMINE.enviroTab);
		fireTorch = new BlockFireTorch(true).setTickRandomly(true).setBlockName("torch").setBlockTextureName("torch_on").setLightLevel(0.9375F).setCreativeTab(EMINE.enviroTab);
		offTorch = new BlockFireTorch(false).setTickRandomly(false).setBlockName("torch").setBlockTextureName("torch_on").setLightLevel(0F).setCreativeTab(EMINE.enviroTab);
		esky = new BlockEsky(Material.iron).setBlockName("enviromine.esky").setCreativeTab(EMINE.enviroTab);
		freezer = new BlockFreezer(Material.iron).setBlockName("enviromine.freezer").setCreativeTab(EMINE.enviroTab);
	...
	public static void registerEntities()
	{
		...
		EntityRegistry.registerModEntity(EntityPhysicsBlock.class, "EnviroPhysicsEntity", physID, EMINE.instance, 64, 1, true);
	...
	public static void LoadIgnitionSources(String[] listIn)
	{
		for(String source : listIn)
		{
			try
			{
				...
				EMINE.logger.log(Level.INFO, "Registered "+ sBlock.getLocalizedName() +"("+source+") as an ignition source.");
			}catch(NullPointerException e)
			{
				EMINE.logger.log(Level.ERROR, "Could not find "+ source);
			}
