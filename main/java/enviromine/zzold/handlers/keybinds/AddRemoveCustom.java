...
{	
	...
	public static void doAddRemove()
	{
		...
		if((!(Minecraft.getMinecraft().isSingleplayer()) || !EMINE.proxy.isClient()) && Minecraft.getMinecraft().thePlayer != null)
		{
		...
		if(mc.currentScreen == null)
		{
			if(Keyboard.isKeyDown(Keyboard.KEY_LSHIFT))
			{
				try
				{
					...
					if(type.name() == "ENTITY")
					{
						...
						} else
						{
							...
							EMINE.logger.log(Level.WARN, "Failed to add config entry. " + lookingAt.getCommandSenderName() + " has no ID!");
						}
				...
				{
					EMINE.logger.log(Level.WARN, "A NullPointerException occured while adding config entry!", e);
				}
			}
			else
			{
