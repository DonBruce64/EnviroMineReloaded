...
public class EM_StatusManager
{
	...
	public static void updateTracker(EnviroDataTracker tracker)
	{
		...
		if(EMINE.proxy.isClient() && Minecraft.getMinecraft().isIntegratedServerRunning())
		{
			if(Minecraft.getMinecraft().isGamePaused() && !EMINE.proxy.isOpenToLAN())
			{
				return;
			}
		}
	...
	public static void syncMultiplayerTracker(EnviroDataTracker tracker)
	{
		...
		EMINE.instance.network.sendToAllAround(new PacketEnviroMine(pData), new TargetPoint(tracker.trackedEntity.worldObj.provider.dimensionId, tracker.trackedEntity.posX, tracker.trackedEntity.posY, tracker.trackedEntity.posZ, 128D));
	}
	...
	{
		if(EMINE.proxy.isClient() && entityLiving.getCommandSenderName().equals(Minecraft.getMinecraft().thePlayer.getCommandSenderName()) && !timer.isRunning())
		{
			timer.start();
		}
		...
		if(tracker == null)
		{
			EMINE.logger.log(Level.ERROR, "Tracker updating as null! Crash imminent!");
		}
