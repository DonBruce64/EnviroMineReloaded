	...
	public EntityPhysicsBlock(World world)
	{
		...
		if(EM_Settings.entityFailsafe > 0 && !world.isRemote)
		{
			...
			if(entityList.size() >= 1024)
			{
				...
				{
					EMINE.logger.log(Level.WARN, "Entity fail safe activated! Canceling new entities!");
					EMINE.logger.log(Level.WARN, "Location: " + this.posX + "," + this.posY + "," + this.posZ);
					EMINE.logger.log(Level.WARN, "No.: " + entityList.size());
					EM_PhysManager.physSchedule.clear();
					this.setDead();
					return;
				} else if(EM_Settings.entityFailsafe >= 2)
				{
					EMINE.logger.log(Level.ERROR, "Entity fail safe activated! Deleting excess entities!");
					EMINE.logger.log(Level.ERROR, "Location: " + this.posX + "," + this.posY + "," + this.posZ);
					EMINE.logger.log(Level.ERROR, "No.: " + entityList.size());
	...
	public EntityPhysicsBlock(World world, double x, double y, double z, Block block, int meta, boolean update)
	{
		...
		if(EM_Settings.entityFailsafe > 0 && !world.isRemote)
		{
			...
			if(entityList.size() >= 512)
			{
				if(EM_Settings.entityFailsafe == 1)
				{
					EMINE.logger.log(Level.WARN, "Entity fail safe activated: Level 1");
					EMINE.logger.log(Level.WARN, "Location: " + this.posX + "," + this.posY + "," + this.posZ);
					EMINE.logger.log(Level.WARN, "No.: " + entityList.size());
					EM_PhysManager.physSchedule.clear();
					this.setDead();
				...
				{
					EMINE.logger.log(Level.ERROR, "Entity fail safe activated: Level 2");
					EMINE.logger.log(Level.ERROR, "Location: " + this.posX + "," + this.posY + "," + this.posZ);
					EMINE.logger.log(Level.ERROR, "No.: " + entityList.size());
					Iterator<EntityPhysicsBlock> iterator = entityList.iterator();
