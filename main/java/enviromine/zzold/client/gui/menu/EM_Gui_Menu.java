...
public class EM_Gui_Menu extends GuiScreen implements GuiYesNoCallback
{
		...
		if(p_73878_1_) // if true
		{
			...
			if(Desktop.isDesktopSupported() && go)
			{
				try 
				{
					...
					EMINE.logger.log(Level.WARN, "(EM_Gui_Menu) Failed to open Default Browser to: " + url);
				}
			}
	...
	public void drawScreen(int par1, int par2, float par3)
	{
		...
		if(!EMINE.proxy.isClient() && MinecraftServer.getServer().getConfigurationManager().func_152607_e(mc.thePlayer.getGameProfile()) || EMINE.proxy.isClient() )
		{
