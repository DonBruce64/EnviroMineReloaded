...
public class SaveController {
    ...
    public static boolean loadConfig(String name, String dirName) {
        ...
        if (!file.exists()) 
        {
            EMINE.logger.warn("Config load canceled, file ("+ file.getAbsolutePath()  +")does not exist. This is normal for first run.");
            return false;
        } else {
            EMINE.logger.info("Config load successful.");
        }
        try {
    ...
    public static void saveConfig(String name, String dirName) {
        ...
        try {
           ...
           // New HUD Settings will be here
            ...
            EMINE.logger.info("Saved GUI properties");
        } catch (IOException e) {
