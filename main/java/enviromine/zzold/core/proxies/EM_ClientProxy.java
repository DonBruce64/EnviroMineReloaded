	...
	public static void initRenderers()
	{
		...
		try 
		{
			...
			if (Loader.isModLoaded("RenderPlayerAPI"))
			{
				...
				EMINE.logger.log(Level.WARN, "Enviromine Doesn't support Player-API/Render-API yet! Config setting \"Render 3D Gear\" set to false");
		...
		{
			EMINE.logger.log(Level.ERROR, "Tried to Render Enviromine Gear, but Failed! Known issues with  Render Player API.:- "+ e);
		}
		...
		while (tmp.hasNext())
		{
			...
			if (itemArmor instanceof ItemArmor && ((ItemArmor)itemArmor).armorType == 1)
			{
				...
				if(EM_Settings.armorProperties.containsKey(name) && EM_Settings.armorProperties.get(name).allowCamelPack)
				{
					...
					if(iRenderer != null)
					{
						EMINE.logger.log(Level.WARN, "Item " + name + " aready has a custom ItemRenderer associated with it!");
						EMINE.logger.log(Level.WARN, "EnviroMine will be unable to overlay the camel pack UI when attached!");
					} else
					{
