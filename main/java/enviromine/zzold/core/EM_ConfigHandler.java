...
public class EM_ConfigHandler
{
	...
	{	
		...
		if(!profileDir.exists())
		{
			...
			{
				EMINE.logger.log(Level.ERROR, "Unable to create directories for profile", e);
			}
		}
		
		if(!profileDir.exists())
		{
			EMINE.logger.log(Level.ERROR, "Failed to load Profile:"+ profile +". Loading Default");	
			profileDir = new File(defaultProfile + customPath);
			loadedProfile = defaultProfile;
		}else 
		{
			loadedProfile = profilePath + profile +"/";
			EMINE.logger.log(Level.INFO, "Loading Profile: "+ profile);
		}
		...
		EMINE.logger.log(Level.INFO, "Loaded " + EM_Settings.stabilityTypes.size() + " stability types");
		EMINE.logger.log(Level.INFO, "Loaded " + EM_Settings.armorProperties.size() + " armor properties");
		EMINE.logger.log(Level.INFO, "Loaded " + EM_Settings.blockProperties.size() + " block properties");
		EMINE.logger.log(Level.INFO, "Loaded " + EM_Settings.livingProperties.size() + " entity properties");
		EMINE.logger.log(Level.INFO, "Loaded " + EM_Settings.itemProperties.size() + " item properties");
		EMINE.logger.log(Level.INFO, "Loaded " + EM_Settings.rotProperties.size() + " rot properties");
		EMINE.logger.log(Level.INFO, "Loaded " + EM_Settings.biomeProperties.size() + " biome properties");
		EMINE.logger.log(Level.INFO, "Loaded " + EM_Settings.dimensionProperties.size() + " dimension properties");
	}	
	...
	public static int initConfig()
	{
		...
		EMINE.logger.log(Level.INFO, "Loading configs...");
	...
	private static void loadGlobalConfig(File file)
	{
		...
		{
			EMINE.logger.log(Level.WARN, "Failed to load main configuration file!", e);
			return;
		}
	...
	public static void CheckDir(File Dir)
	{
		...
		if(Dir.exists())
		{
			EMINE.logger.log(Level.INFO, "Dir already exist:"+ Dir.getName());
			return;
		}
		...
		try
		{
			...
			dirFlag = Dir.mkdirs();
			EMINE.logger.log(Level.INFO, "Created new Folder "+ Dir.getName());
		} catch(Exception e)
		{
			EMINE.logger.log(Level.ERROR, "Error occured while creating config directory: " + Dir.getAbsolutePath(), e);
		}
		
		if(!dirFlag)
		{
			EMINE.logger.log(Level.ERROR, "Failed to create config directory: " + Dir.getAbsolutePath());
		}
	}
		...
		if(datFile == true)
		{
			...
			for(int x = 0; x < catagory.size(); x++)
			{
				...
				if(!CurCat.isEmpty() && CurCat.contains(Configuration.CATEGORY_SPLITTER))
				{
					...
					{
						EMINE.logger.log(Level.WARN, "Failed to load object " + CurCat);
					}
			...
			{
				e.printStackTrace();
				EMINE.logger.log(Level.ERROR, "FAILED TO LOAD CUSTOM CONFIG: " + customFiles.getName() + "\nNEW SETTINGS WILL BE IGNORED!", e);
				return;
			}
		}
	...
	public static Configuration getConfigFromObj(Object obj)
	{
		...
		{
			e.printStackTrace();
			EMINE.logger.log(Level.WARN, "FAILED TO LOAD Config from OBJECT TO "+ModID+".CFG");
			return null;
	...
	public static String SaveMyCustom(Object obj, Object type)
	{
		...
		{
			e.printStackTrace();
			EMINE.logger.log(Level.WARN, "FAILED TO SAVE NEW OBJECT TO "+ModID+".CFG");
		...
		{		
			...
			} else
			{
				...
				EMINE.logger.log(Level.WARN, "Failed to add config entry. " + en.getCommandSenderName() + " has no ID!");
			}
