...
public class BiomeProperties implements SerialisableProperty, PropertyBase
{
	...
	public void LoadProperty(Configuration config, String category)
	{
		...
		if(EM_Settings.biomeProperties.containsKey(id) && !EM_ConfigHandler.loadedConfigs.contains(filename)) EMINE.logger.log(Level.ERROR, "CONFIG DUPLICATE: Biome ID "+id +" was already added from "+ EM_Settings.biomeProperties.get(id).loadedFrom.toUpperCase() +" and will be overriden by "+ filename.toUpperCase());
	...
	public void GenDefaults()
	{
		...
		for(int p = 0; p < biomeArray.length; p++)
		{
			...
			if(!file.exists())
			{
				...
				{
					EMINE.logger.log(Level.ERROR, "Failed to create file for biome '" + biome.biomeName + "'", e);
					continue;
				}
			}
	...
	public void generateEmpty(Configuration config, Object obj)
	{
		...
		{
			EMINE.logger.log(Level.ERROR, "Tried to register config with non biome object!", new Exception());
			return;
		}
