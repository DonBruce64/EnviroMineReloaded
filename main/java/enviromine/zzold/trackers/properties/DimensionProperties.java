...
public class DimensionProperties implements SerialisableProperty, PropertyBase
{
	...
	public void LoadProperty(Configuration config, String category)
	{
		...
		if(EM_Settings.dimensionProperties.containsKey(id) && !EM_ConfigHandler.loadedConfigs.contains(filename)) EMINE.logger.log(Level.ERROR, "CONFIG DUPLICATE: Dimension ID "+ id +" was already added from "+ EM_Settings.dimensionProperties.get(id).loadedFrom.toUpperCase() +" and will be overriden by "+ filename.toUpperCase());
	...
	public void GenDefaults()
	{
		...
		for(int i = 0; i < dimIDs.length; i++)
		{
			...
			if(!file.exists())
			{
				...
				{
					EMINE.logger.log(Level.ERROR, "Failed to create file for dimension '" + dimension.getDimensionName() + "'", e);
					continue;
				}
			}
	...
	public void generateEmpty(Configuration config, Object obj)
	{
		...
		{
			EMINE.logger.log(Level.ERROR, "Tried to register config with non WorldProvider object!", new Exception());
			return;
		}
