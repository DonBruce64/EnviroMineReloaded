...
public class EntityProperties implements SerialisableProperty, PropertyBase
{
	...
	public void LoadProperty(Configuration config, String category)
	{
		...
		if(EM_Settings.livingProperties.containsKey(id) && !EM_ConfigHandler.loadedConfigs.contains(filename)) EMINE.logger.log(Level.ERROR, "CONFIG DUPLICATE: Entity ID "+ id +" was already added from "+ EM_Settings.livingProperties.get(id).loadedFrom.toUpperCase() +" and will be overriden by "+ filename.toUpperCase());
	...
	public void GenDefaults()
	{
		...
		while(iterator.hasNext())
		{
			...
			if(!file.exists())
			{
				...
				{
					EMINE.logger.log(Level.ERROR, "Failed to create file for default entities", e);
					continue;
				}
			}
	...
	public void generateEmpty(Configuration config, Object obj)
	{
		...
		{
			EMINE.logger.log(Level.ERROR, "Tried to register config with non EntityLivingBase id!", new Exception());
			return;
		}
