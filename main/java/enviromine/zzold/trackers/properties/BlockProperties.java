...
public class BlockProperties implements SerialisableProperty, PropertyBase
{
	...
	public void LoadProperty(Configuration config, String category)
	{
		...
		{
			EMINE.logger.log(Level.WARN, "Stability type '" + stability + "' not found.");
			minFall = 99;
			maxFall = 99;
			supportDist = 9;
			hasPhys = false;
		...
		if(metaData < 0)
		{
			...
			if(EM_Settings.blockProperties.containsKey("" + name) && !EM_ConfigHandler.loadedConfigs.contains(filename)) EMINE.logger.log(Level.ERROR, "CONFIG DUPLICATE: Block - "+name.toUpperCase() +" was already added from "+ EM_Settings.blockProperties.get(name).loadedFrom.toUpperCase() +" and will be overriden by "+ filename.toUpperCase());

			EM_Settings.blockProperties.put("" + name, entry);
		} else
		{
			// If item already exist and current file hasn't completely been loaded do this
			if(EM_Settings.blockProperties.containsKey("" + name + "," + metaData) && !EM_ConfigHandler.loadedConfigs.contains(filename)) EMINE.logger.log(Level.ERROR, "CONFIG DUPLICATE: Block - "+name.toUpperCase() +" - Meta:"+ meta +"  was already added from "+ EM_Settings.blockProperties.get(name).loadedFrom.toUpperCase() +" and will be overriden by "+ filename.toUpperCase());
	...
	public void GenDefaults()
	{
		...
		while(iterator.hasNext())
		{
			...
			if(regName.length <= 0)
			{
				EMINE.logger.log(Level.ERROR, "Failed to get correctly formatted object name for " + block.getUnlocalizedName());
				continue;
			}
			...
			if(!blockFile.exists())
			{
				...
				{
					EMINE.logger.log(Level.ERROR, "Failed to create file for " + regName[1], e);
					continue;
				}
			}
	...
	public void generateEmpty(Configuration config, Object obj)
	{
		...
		{
			EMINE.logger.log(Level.ERROR, "Tried to register config with non block object!", new Exception());
			return;
		}
		...
		if(regName.length <= 0)
		{
			EMINE.logger.log(Level.ERROR, "Failed to get correctly formatted object name for " + block.getUnlocalizedName() +"_"+ regName[1]);
			return;
		}
