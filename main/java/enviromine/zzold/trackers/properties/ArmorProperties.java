...
public class ArmorProperties implements SerialisableProperty, PropertyBase
{
	...
	public void LoadProperty(Configuration config, String category)
	{
		...
		if(EM_Settings.armorProperties.containsKey(name) && !EM_ConfigHandler.loadedConfigs.contains(filename)) EMINE.logger.log(Level.ERROR, "CONFIG DUPLICATE: Armor - "+ name.toUpperCase() +" was already added from "+ EM_Settings.armorProperties.get(name).loadedFrom.toUpperCase() +" and will be overriden by "+ filename.toUpperCase());
	...
	public void GenDefaults()
	{
		...
		while(itemList.hasNext())
		{
			...
			if(regName.length <= 0)
			{
				EMINE.logger.log(Level.ERROR, "Failed to get correctly formatted object name for " + armor.getUnlocalizedName());
				continue;
			}
			...
			if(!armorFile.exists())
			{
				...
				{
					EMINE.logger.log(Level.ERROR, "Failed to create file for " + armor.getUnlocalizedName(), e);
					continue;
				}
			}
	...
	public void generateEmpty(Configuration config, Object obj)
	{
		...
		{
			EMINE.logger.log(Level.ERROR, "Tried to register config with non armor object!", new Exception());
			return;
		}
