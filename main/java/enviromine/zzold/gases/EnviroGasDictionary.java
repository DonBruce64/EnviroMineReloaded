...
public class EnviroGasDictionary
{
	...
	public static void addNewGas(EnviroGas newGas, int id)
	{
		if(id < 128 && id >= 0)
		{
			...
			{
				gasList[id] = newGas;
				EMINE.logger.log(Level.INFO, "Registered gas " + newGas.name);
			} else
			{
				EMINE.logger.log(Level.WARN, "Unable to add gas " + newGas.name + " to dictionary: ID " + id + " is used!");
			}
		} else
		{
			EMINE.logger.log(Level.WARN, "Unable to add gas " + newGas.name + " to dictionary: ID out of bounds!");
		}
	}
}
