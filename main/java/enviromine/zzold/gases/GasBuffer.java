...
public class GasBuffer
{
	...
	public static void scheduleUpdate(World world, int x, int y, int z, BlockGas block)
	{
		if(world.isRemote)
		{
			EMINE.logger.log(Level.WARN, "Tried to register gas update in local world!");
			return;
		}
	...
	public static void update()
	{
		...
		if(EMINE.proxy.isClient() && debugTime >= debugInterval && timer.isRunning())
		{
			timer.stop();
			...
			timer.reset();
			debugTime = 0;
		} else if(EMINE.proxy.isClient() && curTick == 1)
		{
			debugTime += 1;
		}
	}
